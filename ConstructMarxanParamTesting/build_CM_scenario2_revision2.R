...
# construct the puvsp.dat file
# use probability of occurrence >= 90% for species
write('species,pu,amount',file=paste0(sWorkDir,"/marxan_scenario2/input/puvsp_pr90.dat"))
for (i in (1:iPuCount)) # for each planning unit
{
  iSpId <- 0
      ...
      if (rAmount > 0)
      {
        ...
        if (rAmount > 0)
        {
          write(paste(iSpId,           # species id
                      ...
                      rAmount,         # amount
                      sep=","),
                file=paste0(sWorkDir,"/marxan_scenario2/input/puvsp_pr90.dat"),
                append=TRUE)
        }
      }
    }
...
# use probability of occurrence >= 90% for species
write('species,pu,amount',file=paste0(sWorkDir,"/marxan_scenario2/input/puvsp_pr90_sporder.dat"))
iSpId <- 0
for (j in (7:iColCount)) # for each conservation feature
{
  iSpId <- iSpId + 1
      ...
      if (rAmount > 0)
      {
        ...
        if (rAmount > 0)
        {
          write(paste(iSpId,           # species id
                      ...
                      rAmount,         # amount
                      sep=","),
                file=paste0(sWorkDir,"/marxan_scenario2/input/puvsp_pr90_sporder.dat"),
                append=TRUE)
        }
      }
    }
