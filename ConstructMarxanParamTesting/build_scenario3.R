...
pu_x_bath10m_table$MARXA_110 > 0
...
write.dbf(pu_join,paste0(sWorkDir,"scenario3_tabarea_tables/pu_join.dbf"))
...
write.csv(pudat,paste0(sWorkDir,"/marxan_scenario3/input/pu.dat"),quote=FALSE,row.names=FALSE)
...
specname <- read.csv(paste0(sWorkDir,"conservation_feature_names_scenario3.csv"))
...
write('id,prop,spf,name',file=paste0(sWorkDir,"/marxan_scenario3/input/spec.dat"))
for (j in (3:iColCount)) # for each conservation feature
{
  write(paste(substr(colnames(pu_join)[j],7,nchar(colnames(pu_join)[j])), # species id
        ...
        sep=","),
        file=paste0(sWorkDir,"/marxan_scenario3/input/spec.dat"),
        append=TRUE)
}

# write the puvsp.dat to disk
write('species,pu,amount',file=paste0(sWorkDir,"/marxan_scenario3/input/puvsp.dat"))
    ...
    if (is.na(pu_join[i,j]) != TRUE)
    {
      if (pu_join[i,j] > 0)
      {
        write(paste(substr(colnames(pu_join)[j],7,nchar(colnames(pu_join)[j])), # species id
                    ...
                    sep=","),
              file=paste0(sWorkDir,"/marxan_scenario3/input/puvsp.dat"),
              append=TRUE)
      }
    }
  }
...
write('species,pu,amount',file=paste0(sWorkDir,"/marxan_scenario3/input/puvsp_sporder.dat"))
for (j in (3:iColCount)) # for each conservation feature
{
  for (i in (1:iRowCount)) # for each planning unit
  {
    if (is.na(pu_join[i,j]) != TRUE)
    {
      if (pu_join[i,j] > 0)
      {
        write(paste(substr(colnames(pu_join)[j],7,nchar(colnames(pu_join)[j])), # species id
                    ...
                    (pu_join[i,j]/10000),                                       # amount (hectares)
                    sep=","),
              file=paste0(sWorkDir,"/marxan_scenario3/input/puvsp_sporder.dat"),
              append=TRUE)
      }
    }
  }
