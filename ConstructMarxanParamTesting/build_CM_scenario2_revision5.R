...
write('speciesid,speciesname,min,max,mean,sd,var',file=paste0(sWorkDir,"/marxan_scenario2_revision4/species_stats.csv"))
for (i in 7:593)
{
  write(paste(i-6,
              ...
              var(pu_matrix[,i]),
              sep=","),
        file=paste0(sWorkDir,"/marxan_scenario2_revision4/species_stats.csv"),
        append=TRUE)
}

# determine the range of values for the costs
write('costid,costname,min,max,mean,sd,var',file=paste0(sWorkDir,"/marxan_scenario2_revision4/costs_stats.csv"))
for (i in 594:609)
{
  write(paste(i-593,
              colnames(pu_matrix)[i],
              ...
              var(pu_matrix[,i]),
              sep=","),
        file=paste0(sWorkDir,"/marxan_scenario2_revision4/costs_stats.csv"),
        append=TRUE)
}
...
write('id,prop,spf,name',file=paste0(sWorkDir,"/marxan_scenario2_revision4/input/spec.dat"))
# we start the new feature codes at 100.
# this is to account for the scenario 1 features that have codes from 4 to 81
iSpId <- 99
for (j in (7:iColCount)) # for each conservation feature
{
  ...
  write(paste(iSpId,rProp,rSPF,colnames(pu_matrix)[j],sep=","),
        file=paste0(sWorkDir,"/marxan_scenario2_revision4/input/spec.dat"),
        append=TRUE)
}

# construct the puvsp.dat file
write('species,pu,amount',file=paste0(sWorkDir,"/marxan_scenario2_revision4/input/puvsp.dat"))
    ...
    if (is.na(pu_matrix[i,j]) != TRUE)
    {
      if (pu_matrix[i,j] > 0)
      {
        write(paste(iSpId,           # species id
                    ...
                    sep=","),
              file=paste0(sWorkDir,"/marxan_scenario2_revision4/input/puvsp.dat"),
              append=TRUE)
      }
    }
  }
...
write('species,pu,amount',file=paste0(sWorkDir,"/marxan_scenario2_revision4/input/puvsp_sporder.dat"))
iSpId <- 99
for (j in (7:iColCount)) # for each conservation feature
{
  iSpId <- iSpId + 1
    ...
    if (is.na(pu_matrix[i,j]) != TRUE)
    {
      ...
      if (rAmount > 0)
      {
          write(paste(iSpId,           # species id
                      ...
                      rAmount,         # amount
                      sep=","),
                file=paste0(sWorkDir,"/marxan_scenario2_revision4/input/puvsp_sporder.dat"),
                append=TRUE)
      }
    }
  }
...
load(file=paste0(sWorkDir,"/marxan_scenario2/input/pu_cost_norm.Rdata"))
...
# add the planning unit status: state protected areas
state_pas <- read.dbf(paste0(sWorkDir,"notake/punotake.dbf"))
head(state_pas)

state_pas <- sqldf("SELECT pu_id FROM state_pas WHERE MAX_strict > 0")
dim(state_pas)

for (i in 1:nrow(state_pas))
{
  iWhich <- which(pudat$id==state_pas$pu_id[i])
