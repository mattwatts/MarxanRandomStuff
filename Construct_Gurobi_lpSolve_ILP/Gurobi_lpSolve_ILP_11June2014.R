#################
# Gurobi: scenario 2
# Note: we have added status to pu.dat for some pu's locked in and locked out
...
# parse the costs and the matrix and the pu status,
# only include rows that have cost > 0, feature amounts > 0, and pu status == 0
...
for (i in 1:length(puid_))
{
  ...
  if (pudat$status[i] == 2)
  {
    rReservedCost <- rReservedCost + gcost_[i]
  }
  if (pudat$status[i] == 3)
  {
    rExcludedCost <- rExcludedCost + gcost_[i]
  }
...
# remember the planning units filtered because of no features, and their cost
write(paste0('filtered cost ',rFilteredCost),file=paste0(sMarxanDir,"/input/filtered_cost.txt"))
write(paste0('reserved cost ',rReservedCost),file=paste0(sMarxanDir,"/input/filtered_cost.txt"),append=TRUE)
write(paste0('excluded cost ',rExcludedCost),file=paste0(sMarxanDir,"/input/filtered_cost.txt"),append=TRUE)
...
params <- list(Presolve=0,TimeLimit=10.0)
...
#lp.control(f, timeout = 10, verbose = "detailed")
